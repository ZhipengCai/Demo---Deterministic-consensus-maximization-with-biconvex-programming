function [M, inliers, iter, outSampleSet] = myRANSAC(x, fittingfn, distfn, degenfn, s, t, method, inpSampleSet, feedback, maxDataTrials, maxTrials, matchingScores)
...
while (N > trialcount || trialcount < 100)%||(trialcount <= size(inpSampleSet,1))
    ...
    if (strcmp(method, 'LORANSAC') || strcmp(method, 'F-LORANSAC')) % prevent early exit for LORANSAC and FIXING-LORANSAC
        N = trialcount  + 1;
    end
    ...
    if trialcount >= maxTrials
        disp([method 'reaches the maximum number of ' num2str(maxTrials) 'iterations']);
        break;
    end
