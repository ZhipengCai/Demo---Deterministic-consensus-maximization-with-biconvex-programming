function [theta, runtime] = IBCO_Ori(A, b, c, d, theta0, th, config,problem)
if nargin < 8
    problem = 'general';
end
...
%modelsx = genModelSocp_BCO(A,b,c,d,epsilon_ac,NoPoints,d1,d2);
...
while deltaMax>(deltaMin+1)
    ...
    if fDelta <= config.QThresh && update == 1
        ...
        p = floor(delta - fDelta*(delta-deltaMin)/(fDelta-fDeltaMinProj));
        deltaMin = delta;
        fDeltaMinProj = fDelta;
