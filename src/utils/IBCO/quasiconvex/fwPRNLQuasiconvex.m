...
function [xn, yn, theta,  currF] = fwPRNLQuasiconvex(A, b, cd,dd, x0, y0, delta, config, denThresh) %Note: cd, dd are to enforce strictly cheirality contraints
    ...
    while (true)
        ...
        fr = N11*yip+b;        
        [rip, ~] = solve_u_lpPRNL_Homo(fr,delta);                                
    ...
    function [F] = evalF(r, s, y)        
        F = abs(r'*(s+N11*y+b) + s'*(-r+ones(n,1)));      
    end
