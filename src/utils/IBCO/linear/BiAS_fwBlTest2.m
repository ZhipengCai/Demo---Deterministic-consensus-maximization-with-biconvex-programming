...
function [xn, yn, theta, currFVec, runtimeVec] = BiAS_fwBlTest2(A, b, x0, y0, delta, lpsolver)
...
if delta == 0
   ...
   runtimeVec = [toc];
    ...
    for iter = 1:maxIter  
        ...
        if convergeX == 0 && (convergeY==1||xOry == 0)
            %disp('entering x');
            if bland == 0
