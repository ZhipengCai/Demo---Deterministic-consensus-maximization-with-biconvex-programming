function [theta, inls, nInls, runtime] = myParaRefFW(x, y, th, theta0, config)
        ...
        [x1, y1, theta0, fDeltaMin] = BiAS_fwBl1(A, b, x0, y0, 0, config.lpsolver);
	...
	%judge whether current solution is better
        ...
        while deltaMax>(deltaMin+1)
            %step 1:
            [x1, y1, theta0, fDelta] = BiAS_fwBl1(A, b, x0, y0, delta, config.lpsolver);
            inls = find(abs(y-x*theta0)<=th);
            nInls = numel(inls);
            %step 2:
            if deltaMax>=(NoPoints-nInls)
