...
class FundMatrixEstimator: public USAC<FundMatrixEstimator>
{
	...
	public:
		...
		inline bool 	 enforceScale(unsigned int modelIndex); //enforcing the scale of fundamental matrix by setting the 8-th entry to 1
...
bool FundMatrixEstimator::enforceScale(unsigned int modelIndex)
{
	double tmp = *(models_denorm_[modelIndex]+7);
	if(fabs(tmp)<1e-9){
		tmp = 1e9;	
		*(models_denorm_[modelIndex]+7) = 1e9;	
	}
	for(unsigned int i = 0; i<9;i++)
	{
		*(models_denorm_[modelIndex]+i)/=tmp;
	}
	return true;
}
...
void FundMatrixEstimator::storeModel(unsigned int modelIndex, unsigned int numInliers)
{
	...
	for (unsigned int i = 0; i < 9; ++i)
	{
		//final_model_params_[i] = *(models_denorm_[modelIndex]+i)/(*(models_denorm_[modelIndex]+7));
