...
bool HomogEstimator::evaluateModel(unsigned int modelIndex, unsigned int* numInliers, unsigned int* numPointsTested)
{
	...
	*numPointsTested = 0;
	...
	for (unsigned int i = 0; i < usac_num_data_points_; ++i)
	{
		...
		//compute transfer error on the target image	
		...
		for (unsigned int j = 0; j < 2; ++j)
		{
			...
			//plot h_x and pt
			//printf("h_x[%d] = %f; h_x[2] = %f; pt[%d] = %f; pt[2] = %f; ptp[%d] = %f; ptp[2] = %f\n",
			// j, h_x[j], h_x[2],j, pt[j],pt[2],j,pt[3+j],pt[3+2]);
			//printf("error[%d] = %f\n",j,h_x[j]/h_x[2] - pt[3+j]);
